...
function createMessageRowElement(msg, isSos = false) {
  ...
  if (parsedContent && parsedContent.type === 'image') {
    ...
    if (ctrStatus === 'accepted') { statusPillClass = 'contract-status-accepted'; statusLabel = 'En cours'; }
    else if (ctrStatus === 'adjustment_requested') { statusPillClass = 'contract-status-adjustment'; statusLabel = 'À ajuster'; }
    else if (ctrStatus === 'completed') { statusPillClass = 'contract-status-completed'; statusLabel = 'Terminé'; }
    else if (ctrStatus === 'cancelled') { statusPillClass = 'contract-status-cancelled'; statusLabel = 'Annulé'; }
      ...
      <div class="chat-contract-card-premium status-${ctrStatus}">
        <div class="contract-card-header-bar">
          <div class="contract-card-badge-pill">
            <span>MICRO-CONTRAT</span>
          </div>
          <div style="display: flex; align-items: center; gap: 6px;">
            <span class="contract-card-status-pill ${statusPillClass}">${statusLabel}</span>
            <button type="button" class="btn-contract-delete-sm" onclick="window.deleteDirectContract('${ctrId}')" title="Supprimer">
        ...
        <div class="contract-box-title">
          ...
          ${parsedContent.actionNote ? `<div class="contract-note-text"><strong>Note :</strong> "${escapeHtml(parsedContent.actionNote)}"</div>` : ''}
        </div>

        <div class="contract-meta-row">
          <div class="contract-meta-item">
            <span class="contract-meta-label">Montant</span>
            <span class="contract-meta-val amount">${ctrAmount}</span>
          </div>
          <div class="contract-meta-item">
            <span class="contract-meta-label">Échéance</span>
            <span class="contract-meta-val">${ctrDeadline}</span>
          </div>
        </div>
        ...
        <div class="task-card-body">
          ...
          <div class="task-card-details">
            ${isCreated ? `
              ...
              <span class="task-detail-item">Statut initial : <strong class="status-${statusTheme}-text">${escapedStatusLbl}</strong></span>
            ` : `
              <span class="task-detail-item">Modifiée par : <strong>${creator}</strong></span>
              <span class="task-detail-item">Nouveau statut : <strong class="status-${statusTheme}-text">${escapedStatusLbl}</strong></span>
            `}
          </div>
        </div>
...
window.openPinnedMessagesListModal = function() {
  ...
  if (msgs.length === 0) {
    ...
    container.innerHTML = msgs.map((msg, idx) => {
      ...
      const targetId = msg.message_id || msg.id;
        ...
        <div class="pinned-msg-item-card ${isActive ? 'active-pinned-card' : ''}">
          <div class="pinned-msg-item-header">
            ...
            <div class="pinned-msg-item-actions">
              <button type="button" class="pinned-btn-jump" onclick="window.jumpToPinnedMessage('${targetId}')" title="Voir dans le chat">
              ...
              <button type="button" class="pinned-btn-unpin" onclick="window.unpinSpecificMessage('${targetId}')" title="Dépingler ce message">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
              </button>
            </div>
          </div>
          <div class="pinned-msg-item-preview" onclick="window.jumpToPinnedMessage('${targetId}')">${escapeHtml(preview)}</div>
        </div>
      `;
    }).join('');
  }
...
window.updatePinnedMessageBanner = function(channelType, targetId, pinnedMessages = []) {
  ...
  const activeMsgId = currentMsg.message_id || currentMsg.id;
  
  // Clean text preview (NO SENDER NAME, WhatsApp style)
  let rawText = currentMsg.content || '';
  try {
    const parsed = JSON.parse(rawText);
    if (parsed.text) rawText = parsed.text;
    else if (parsed.fileName) rawText = `📎 ${parsed.fileName}`;
  ...
  if (rawText.length > 55) {
    rawText = rawText.substring(0, 55) + '...';
  }

  // Pure WhatsApp layout: only message content text, no author name
  if (bannerText) {
  ...
  if (btnNext) {
    if (msgs.length > 1) {
      ...
      btnNext.onclick = (e) => {
        ...
        const nextMsg = msgs[state.pinnedCurrentIndex];
        if (nextMsg) {
          window.jumpToPinnedMessage(nextMsg.message_id || nextMsg.id);
        }
      };
    } else {
  ...
  if (btnUnpin) {
    ...
    btnUnpin.onclick = (e) => {
      e.stopPropagation();
      window.pinCurrentChatMessage(activeMsgId, 'unpin');
    };
  }
};
...
function renderDirectContracts(contracts) {
  ...
  container.innerHTML = contracts.map(c => {
    ...
    if (c.status === 'accepted') { badgeClass = 'badge-accepted'; statusText = 'En cours'; }
    else if (c.status === 'adjustment_requested') { badgeClass = 'badge-adjustment'; statusText = 'À ajuster'; }
